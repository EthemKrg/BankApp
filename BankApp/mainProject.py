...
# arkaplandaki pyhton fonksiyonlarını başlatıyoruz
def is_valid(customer_account_number):
    try:
        ...
        print("# Customer database'i bulunmuyor!\n# :Yeni customer database'i oluşturuldu.")
        customer_database = open("./database/Customer/customerDatabase.txt", "a")

    else:  # Eğer müşteri numarası alınmışsa false döndür, alınmamışsa true.
...
def is_valid_mobile(mobile_number): # 10 birim uzunlugunda olmali
...
def display_account_summary(identity, choice):  # choice 1 for full summary; choice 2 for only account balance.
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            if choice == 1:
                output_message += "HESAP NUMARASI : " + line.replace("\n", "") + "\n"
                customer_database.__next__()  # skipping pin
                output_message += "VARLIK : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "HESABI OLUŞTURMA TARİHİ : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "HESABIN SAHİBİ : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "HESAP TÜRÜ : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "DOĞUM TARİHİ : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "TELEFON NUMARASI : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "CİNSİYET : " + customer_database.__next__().replace("\n", "") + "\n"
                output_message += "IRK : " + customer_database.__next__().replace("\n", "") + "\n"
                #output_message += "KYC : " + customer_database.__next__().replace("\n", "") + "\n"
            else:
                customer_database.readline()  # skipped pin
                output_message += "VARLIK : " + customer_database.readline().replace("\n", "") + "\n"
            flag = 1
            break
...
def delete_customer_account(identity, choice):  # choice 1 for admin, choice 2 for customer
    ...
    if flag == 1:
        output_message = str(identity) + " Numaralı hesap başarıyla kapatıldı!"
        if choice == 1:
            adminMenu.printMessage_outside(output_message)
        print(output_message)
    else:
        output_message = "Hesap bulunamadı !"
        if choice == 1:
            adminMenu.printMessage_outside(output_message)
        print(output_message)
...
def create_admin_account(identity, password):
    ...
    output_message = "Admin hesabı başarıyla oluşturuldu!"
    adminMenu.printMessage_outside(output_message)
    print(output_message)
...
def delete_admin_account(identity):
    ...
    if flag == 1:
        output_message = identity + "Numaralı hesap başarıyla kapatıldı!"
        print(output_message)
        adminMenu.printMessage_outside(output_message)
    else:
        output_message = "Hesap bulunamadı :("
...
def change_PIN(identity, new_PIN):
    ...
    output_message = "PIN başarıyla değiştirildi."
...
def transaction(identity, amount, choice):  # choice 1 for deposit; choice 2 for withdraw
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            ...
            if choice == 2 and balance - amount < 0:  # Minimum balance 10000
                return -1
            else:
                if choice == 1:
                    balance += amount
...
def check_credentials(identity, password, choice,
    ...
    try:
        ...
        print("#", folder_name[2:], "database bulunamadı!\n# New", folder_name[2:],
              "database otomatik olarak oluşturuldu.")
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("E&E BANKASINA HOŞ GELDİNİZ")
        p1 = PhotoImage(file='./images/bank1.png')
        window.iconphoto(True, p1)
        window.configure(background="#976AF7")
        window.configure(cursor="arrow")

        self.Canvas1 = tk.Canvas(window, background="#F7D577", borderwidth="0", insertbackground="black",
                                 relief="ridge",
        ...
        self.Button1 = tk.Button(self.Canvas1, command=self.selectEmployee, activebackground="#ececec",
                                 activeforeground="#FFFFFF", background="#83DAF7", disabledforeground="#a3a3a3",
        ...
        self.Label1 = tk.Label(self.Canvas1, background="#FFE791", disabledforeground="#a3a3a3",
                               font="-family {Segoe UI} -size 13 -weight bold", foreground="white",
                               text='''LÜTFEN SEÇİM YAPINIZ''')
...
class adminLogin:
    def __init__(self, window=None):
        ...
        global Canvas1
        Canvas1 = tk.Canvas(window, background="#F7D577", insertbackground="black", relief="ridge",
                            selectbackground="blue", selectforeground="white")
        Canvas1.place(relx=0.108, rely=0.142, relheight=0.715, relwidth=0.798)

        self.Label1 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3",
                               font="-family {Segoe UI} -size 14 -weight bold", foreground="#00254a",
                               text="ADMİN GİRİŞİ")
        self.Label1.place(relx=0.135, rely=0.142, height=41, width=154)

        global Label2
        Label2 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#F7D577", foreground="#F7D577")
        ...
        self.Label3 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3", foreground="#000000")
        ...
        self.Label4 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3", foreground="#000000")
        ...
        self.Label5 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3", foreground="#000000")
        ...
        self.Button = tk.Button(Canvas1, text="GİRİŞ", borderwidth="0", width=10, background="#83DAF7",
                                foreground="#00254a",
                                font="-family {Segoe UI} -size 10 -weight bold",
                                command=lambda: self.login(self.Entry1.get(), self.Entry1_1.get()))
        self.Button.place(relx=0.765, rely=0.755)

        self.Button_back = tk.Button(Canvas1, text="GERİ", borderwidth="0", width=10, background="#6399AB",
                                     foreground="#00254a",
                                     font="-family {Segoe UI} -size 10 -weight bold",
                                     command=self.back)
    ...
    def setImg():
        ...
        Label2.place(relx=0.067, rely=0.283, height=200, width=250)
    ...
    def login(self, admin_id, admin_password):
        ...
        if check_credentials(admin_id, admin_password, 1, True):
            ...
            Error.setMessage(self, message_shown=" BİLGİLER GEÇERSİZ!")
            self.setImg()
...
class CustomerLogin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("MÜŞTERİ")
        window.configure(background="#976AF7")

        global Canvas1
        Canvas1 = tk.Canvas(window, background="#F7D577", insertbackground="white", relief="ridge",
                            selectbackground="blue", selectforeground="white")
        Canvas1.place(relx=0.108, rely=0.142, relheight=0.715, relwidth=0.798)

        Label1 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3",
                          font="-family {Segoe UI} -size 14 -weight bold", foreground="#00254a",
                          text="MÜŞTERİ GİRİŞİ")
        Label1.place(relx=0.135, rely=0.142, height=41, width=154)

        global Label2
        Label2 = tk.Label(Canvas1, background="#F7D577", disabledforeground="#a3a3a3", foreground="#000000")
        Label2.place(relx=0.067, rely=0.283, height=181, width=233)
        global _img0
        _img0 = tk.PhotoImage(file="./images/customer.png")
        Label2.configure(image=_img0)

        self.Entry1 = tk.Entry(Canvas1, background="#e2e2e2", borderwidth="2", disabledforeground="#a3a3a3", #textbox ın içi
                               font="TkFixedFont", foreground="#000000", highlightbackground="#b6b6b6",
                               highlightcolor="#004080", insertbackground="black")
        self.Entry1.place(relx=0.607, rely=0.453, height=20, relwidth=0.26)
        ...
        global _img2
        _img2 = tk.PhotoImage(file="./images/lock1.png")
        self.Label4.configure(image=_img2, background="#F7D577")
    ...
    def login(self, customer_account_number, customer_PIN):
        if check_credentials(customer_account_number, customer_PIN, 2, False):
            ...
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="GEÇERSİZ GİRİŞ BİLGİSİ!")
            self.setImg()
...
class adminMenu:
    def __init__(self, window=None):
        ...
        self.Button1 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#83DAF7", borderwidth="0", disabledforeground="#a3a3a3",
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="HESABI KAPAT", command=self.closeAccount)
        ...
        self.Button2 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="YENİ HESAP", command=self.createCustaccount)
        ...
        self.Button3 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#83DAF7", borderwidth="0", disabledforeground="#a3a3a3",
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text="ÇIKIŞ",
                                 command=self.exit)
        ...
        self.Button4 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="ADMİN HESABI OLUŞTUR", command=self.createAdmin)
        ...
        self.Button5 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="ADMİN HESABINI KAPAT", command=self.deleteAdmin)
        ...
        self.Button6 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 background="#83DAF7", foreground="#fffffe", borderwidth="0",
                                 disabledforeground="#a3a3a3", font="-family {Segoe UI} -size 11",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text="HESAP DURUMU ", command=self.showAccountSummary)
        self.Button6.place(relx=0.04, rely=0.683, height=34, width=181, bordermode='ignore')

        global Frame1
        Frame1 = tk.Frame(window, relief='groove', borderwidth="2", background="#F7D577")
...
class CloseAccountByAdmin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("MÜŞTERİ HESABINI KAPAT")
        window.configure(background="#f2f3f4")
        ...
        self.Button1 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", borderwidth="0",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text="GERİ",
                                 command=self.back)
        ...
        self.Button2 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 ...
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text="DEVAM ET",
    ...
    def submit(self, identity):
        if not is_valid(identity):
            ...
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="HESAP BULUNAMADI!")
            return
        self.master.withdraw()
...
class createCustomerAccount:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("YENİ HESAP OLUŞTUR")
        window.configure(background="#f2f3f4")
        ...
        self.Label1 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               ...
                               highlightcolor="black", text='''HESAP NUMARASI:''')
        ...
        self.Label2 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''İSİM:''')
        ...
        self.Label3 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''HESAP TÜRÜ:''')
        ...
        self.Radiobutton1 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           ...
                                           text='''BİRİKİM''', variable=acc_type, value="Savings")
        ...
        self.Radiobutton1_1 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                             background="#f2f3f4", disabledforeground="#a3a3a3", foreground="#000000",
                                             highlightbackground="#d9d9d9", highlightcolor="black", justify='left',
                                             text='''NORMAL''', variable=acc_type, value="Current")
        ...
        self.Label5 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               ...
                               highlightcolor="black", text='''TELEFON: ''')
        self.Label5.place(relx=0.345, rely=0.323, height=22, width=85)

        self.Label4 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000",
                               highlightcolor="black", text='''DOĞUM TARİHİ (DD/MM/YYYY) :''')
        ...
        self.Label6 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000",
                               highlightcolor="black", text='''CİNSİYET:''')
        self.Label6.place(relx=0.350, rely=0.402, height=15, width=65)
        ...
        self.Radiobutton3 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           ...
                                           text='''ERKEK''', variable=gender, value="Male")
        ...
        self.Radiobutton4 = tk.Radiobutton(window, activebackground="#ececec", activeforeground="#000000",
                                           background="#f2f3f4", disabledforeground="#a3a3a3", foreground="#000000",
                                           highlightbackground="#d9d9d9", highlightcolor="black", justify='left',
                                           text='''KADIN''', variable=gender, value="Female")
        ...
        self.Label7 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''IRK:''')
        self.Label7.place(relx=0.345, rely=0.471, height=21, width=75)
        ...
        self.Label9.place(relx=0.345, rely=0.62, height=21, width=35)

        self.Label10 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                                disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                                highlightcolor="black", text='''PIN TEKRAR:''')
        self.Label10.place(relx=0.292, rely=0.695, height=21, width=85)

        self.Label11 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                                disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                                highlightcolor="black", text='''BAŞLANGIÇ BAKİYESİ:''')
        self.Label11.place(relx=0.292, rely=0.779, height=21, width=85)
        ...
        self.Label8 = tk.Label(window, background="#f2f3f4", disabledforeground="#a3a3a3", foreground="#000000",
                               text='TC NO :  ')
        self.Label8.place(relx=0.345, rely=0.546, height=24, width=50)
                                        ...
                                        if PIN.isnumeric() and PIN.__len__() == 4:
                                            if confirm_PIN == PIN:
                                                if initial_balance.isnumeric():
                                                    ...
                                                    Error(Toplevel(self.master))
                                                    Error.setMessage(self, message_shown="GEÇERSİZ BAKİYE!")
                                                    return
                                            else:
                                                Error(Toplevel(self.master))
                                                Error.setMessage(self, message_shown="PIN UYUŞMUYOR!")
                                                return
                                        else:
                                            Error(Toplevel(self.master))
                                            Error.setMessage(self, message_shown="GEÇERSİZ PIN!")
                                            return
                                    else:
                                        Error(Toplevel(self.master))
                                        Error.setMessage(self, message_shown="KİMLİK YÜKLEYİN!")
                                        return
                                else:
                                    Error(Toplevel(self.master))
                                    Error.setMessage(self, message_shown="IRK GİRİN!")
                                    return
                            else:
                                Error(Toplevel(self.master))
                                Error.setMessage(self, message_shown="CİNSİYET SEÇİN!")
                                return
                        else:
                            Error(Toplevel(self.master))
                            Error.setMessage(self, message_shown="GEÇERSİZ TELEFON NUMARASI!")
                            return
                    else:
                        Error(Toplevel(self.master))
                        Error.setMessage(self, message_shown="GEÇERSİZ TARİH!")
                        return
                else:
                    Error(Toplevel(self.master))
                    Error.setMessage(self, message_shown="HESAP TÜRÜ SEÇİN!")
                    return
            else:
                Error(Toplevel(self.master))
                Error.setMessage(self, message_shown="İSMİNİZİ GİRİN!")
                return
        else:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="HESAP NUMARASI GEÇERSİZ!")
            return
...
class createAdmin:
    def __init__(self, window=None):
        ...
        window.maxsize(1370, 749)
        window.resizable(0, 0)
        window.title("ADMİN HESABI OLUŞTUR")
        window.configure(background="#f2f3f4")
        ...
        self.Label3 = tk.Label(window, activebackground="#f9f9f9", activeforeground="black", background="#f2f3f4",
                               disabledforeground="#a3a3a3", foreground="#000000", highlightbackground="#d9d9d9",
                               highlightcolor="black", text='''PAROLAYI ONAYLA:''')
    ...
    def create_admin_account(self, identity, password, confirm_password):
        if check_credentials(identity, "DO_NOT_CHECK_ADMIN", 1, False):
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="ID GEÇERSİZ!")
        else:
            if password == confirm_password and len(password) != 0:
                ...
                if password != confirm_password:
                    Error.setMessage(self, message_shown="PAROLA UYUŞMUYOR!")
                else:
                    Error.setMessage(self, message_shown="GEÇERSİZ PAROLA!")
...
class deleteAdmin:
    ...
    def delete_admin(self, admin_id, password):
        if admin_id == "aayush" or admin_id == admin_idNO:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="HATA!")
            return
...
class customerMenu:
    def __init__(self, window=None):
        ...
        self.Button3 = tk.Button(self.Labelframe1, command=self.exit, activebackground="#ececec",
                                 ...
                                 foreground="#fffffe", highlightbackground="#83DAF7", highlightcolor="black", pady="0",
                                 text='''ÇIKIŞ''')
        self.Button3.place(relx=0.667, rely=0.683, height=34, width=181, bordermode='ignore')
        ...
        self.Button4 = tk.Button(self.Labelframe1, command=self.selectChangePIN, activebackground="#ececec",
                                 activeforeground="#000000", background="#83DAF7", borderwidth="0",
                                 disabledforeground="#a3a3a3", font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0", text='''PINI DEĞİŞTİR''')
        self.Button4.place(relx=0.04, rely=0.439, height=34, width=181, bordermode='ignore')
        ...
        self.Button6 = tk.Button(self.Labelframe1, activebackground="#ececec", activeforeground="#000000",
                                 ...
                                 font="-family {Segoe UI} -size 11", foreground="#fffffe",
                                 highlightbackground="#d9d9d9", highlightcolor="black", pady="0",
                                 text='''BAKİYEYİ KONTROL ET''', command=self.checkBalance)
        self.Button6.place(relx=0.04, rely=0.683, height=34, width=181, bordermode='ignore')

        global Frame1_1_2
        Frame1_1_2 = tk.Frame(window, relief='groove', borderwidth="2", background="#F7D577")
...
class depositMoney:
    def __init__(self, window=None):
        ...
        self.Button1 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 ...
                                 highlightcolor="black", pady="0", text='''DEVAM ET''',
                                 command=lambda: self.submit(self.Entry1.get()))
    ...
    def submit(self, amount):
        if amount.isnumeric():
            if 25000 >= float(amount) > 0:
                ...
                if float(amount) > 25000:
                    Error.setMessage(self, message_shown="LİMİT AŞILDI!")
                else:
                    Error.setMessage(self, message_shown="GEÇERSİZ MİKTAR!")
                return
        else:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="GEÇERSİZ MİKTAR!")
        ...
        if output == -1:
            ...
            output = "MİKTAR " + str(amount) + " İŞLEM BAŞARILI .\nGÜNCELLENEN MİKTAR : " + str(output)
...
class withdrawMoney:
    ...
    def submit(self, amount):
        if amount.isnumeric():
            if 25000 >= float(amount) > 0:
                ...
                if float(amount) > 25000:
                    ...
                    Error.setMessage(self, message_shown="GEÇERSİZ MİKTARR!")
                return
        else:
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="GEÇERSİZ MİKTAR!")
        ...
        if output == -1:
            ...
            output = "MİKTARY " + str(amount) + " BAŞARIYLA ÇEKİLDİ .\nGÜNCEL BAKİYE : " + str(output)
            customerMenu.printMessage_outside(output)
            self.master.withdraw()
...
class changePIN:
    def __init__(self, window=None):
        ...
        self.Button2 = tk.Button(window, activebackground="#ececec", activeforeground="#000000", background="#004080",
                                 disabledforeground="#a3a3a3", foreground="#ffffff", borderwidth="0",
                                 highlightbackground="#d9d9d9",
                                 highlightcolor="black", pady="0", text="GERİ", command=self.back)
    ...
    def submit(self, new_PIN, confirm_new_PIN):
        if new_PIN == confirm_new_PIN and str(new_PIN).__len__() == 4 and new_PIN.isnumeric():
            ...
            if new_PIN != confirm_new_PIN:
                Error.setMessage(self, message_shown="PİN UYUŞMUYOR!")
            elif str(new_PIN).__len__() != 4:
                Error.setMessage(self, message_shown="PİN 4 BİRİM UZUNLUKTA OLMALI !")
            else:
                Error.setMessage(self, message_shown="GEÇERSİZ PİN!")
            return
...
class checkAccountSummary:
    ...
    def submit(self, identity):
        if not is_valid(identity):
            ...
            Error(Toplevel(self.master))
            Error.setMessage(self, message_shown="ID BULUNAMADI!")
            return
        self.master.withdraw()
